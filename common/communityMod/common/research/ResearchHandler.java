...
public class ResearchHandler {
	 ...
	 * 
	 * The player will research something, and they will be given an item with
	 * metadata. The metatadata corresponds to the research that has been
	 * completed. When the player right-clicks with their research, the item
	 * will send a request to the Research Handler to set that research number
	 * to true, and the player will be able to do everything that research sets
	 * up.
	 */
			...
			if (research.get(accessors.get(i))) {
				if (accessors.get(i) == ItemsHelper.siliconIngot.itemID) {
					GameRegistry.addRecipe(new ItemStack(
							BlocksHelper.siliconBlock, 1),
							new Object[] { "SSS", "SSS", "SSS", 'S',
									ItemsHelper.siliconIngot });
					GameRegistry.addRecipe(new ItemStack(
							ItemsHelper.siliconWafer, 8), new Object[] { "SSS",
							"SNS", "SSS", 'S', ItemsHelper.siliconIngot });

					GameRegistry.addRecipe(new ItemStack(
							ItemsHelper.siliconChip, 4), new Object[] { "SIS",
							"IWI", "SIS", 'S', ItemsHelper.siliconIngot, 'I',
							ItemsHelper.siliconWafer, 'W', Item.bucketWater });
				}
			}
		}
	}
